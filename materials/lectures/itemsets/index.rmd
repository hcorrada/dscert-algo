...
.title[Algorithms for Data Science: Itemsets]
...
$\textrm{min_hash}(S_1)=a$, $\textrm{min_hash}(S_2)=c$

---

## Min-Hash

Property: $Pr(\textrm{min_hash}(S_i) = \textrm{min_hash}(S_j)) = JS(S_i,S_j)$
...
$\mathrm{support}(I)$ the number of baskets in which itemset $I$ appears

Frequent itemsets: Itemsets $I$ with $\mathrm{support}(I)\geq s$ (for some threshold $s$)
...
For $s=3$ what are frequent itemsets of size 1 or 2
---
...
_rule confidence_: $\mathrm{confidence}(I \to j) = \frac{\mathrm{support}(I \bigcup \{j\})}{\mathrm{support}(I)}$

_rule interest_: $\mathrm{confidence}(I \to j) - |\{ B \in \mathrm{Baskets} \textrm{ s.t. } j \in B\}|$

---

## Association Rules

Note: once we have frequent itemsets, we can get association rules with high confidence easily

--

Suppose we find **all** frequent itemsets over some support threshold $s$

Let itemset $J$ with $n$ items be one of those itemsets ( $\mathrm{support}(J) \geq s$ ), then

1. there are only $n$ candidate Association Rules $(J-\{j\}) \rightarrow j$  
2. $\mathrm{support}(J)\geq s$ implies $\mathrm{support}(J-\{j\}) \geq s$ so we must have already calculated the latter
...
Suppose we are given a set of baskets over $n$ items
